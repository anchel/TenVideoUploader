tvu.util = {
	log: function(msg) {
		if (tvu.debug && window.console && window.console.log && window.console.log != window.alert) {
			window.console.log('[tvu] ' + msg);
		}
	},
	 ...
	 * @param  {Object} paramObject 参数对象
	 * @param  {String} baseUrl     URL
	 * @return {[type]}             参数字符串或完整请求URL
	 */
	param: function(paramObject, baseUrl) {
		var paramArr = [],
			paramStr = '',
			sep = '';
		baseUrl = baseUrl || '';
		for (var k in paramObject) {
			if (paramObject.hasOwnProperty(k)) {
				paramArr.push(k + '=' + encodeURIComponent(paramObject[k]));
			}
		}
	...
	getSubParam: function(param, keysArr) {
		var newParam = {},
			tmpKey,
			tmpVal;
		keysArr || (keysArr = []);
		for (var i = 0, len = keysArr.length; i < len; i++) {
			tmpKey = keysArr[i];
			tmpVal = param[tmpKey];
			newParam[tmpKey] = (tmpVal === undefined || tmpVal === null) ? '' : tmpVal;
		}
		return newParam;
	},
	getFileName: function(path, noExt) {
	...
	ajax: function(settings) {
		...
		if (typeof settings.type === 'string' && settings.type.toUpperCase() == 'POST') {
			tvu.util.ajax.post(settings);
		} else {
			tvu.util.ajax.get(settings);
		}
	}
};
...
(function() {
	...
	tvu.util.ajax.get = jsonpObj.get;
}());
...
(function() {
	...
	FormSender._clear = function(o) {
		...
		if (tvu.util.userAgent.safari || tvu.util.userAgent.opera) {
	 ...
	 * 重载ajax组件，支持跨域post请求
	 */
	tvu.util.ajax.post = function(settings) {
		var beforeSend = success = error = complete = function() {}, formSender,
			url, data, charset;
 ...
 */
(function() {
	var ua = tvu.util.userAgent = {}, agent = navigator.userAgent,
